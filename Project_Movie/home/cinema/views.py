...
class AttributeView(APIView):
    def get(self, request):
        ...
        try:
            ...
            return response_failure(400, message='查询对象结果为空')
...
class CinemaViewing(APIView):
            ...
            try:
                if view_id:
                    ...
                    if view:
                        ...
                        if seats is None:
                            return response_failure(code=400, message='该场次下没有座位信息')
                        return response_success(code=200, data=serializer.data)
                    else:
                        return response_failure(code=400, message='当前场次不存在')
                # 根据电影id/日期获取场次信息
                else:
                    ...
                    if movie_id and date_time and cinema_id:
                        ...
                        return response_failure(code=400, message='请求失败，当前影院/电影不存在')
    ...
    def post(self, request):
        ...
        if query_params:
            ...
            if view_info:
                return response_failure(code=400, message='该场次信息已存在')
            try:
            ...
            try:
                ...
                if view is None:
                    ...
                    if serializer.is_valid():
                        ...
                        return response_failure(code=400, message='存储到数据库失败')
            except:
                raise
            return response_success(code=200, data=serializer.data)
        ...
        if query_params:
            ...
            try:
                if view_id:
                    ...
                    return response_failure(code=400, message='请输入正确的场次')
                # 获取座位信息
                ...
                with transaction.atomic():
                    ...
                    if db_seats:
                        ...
                        for seat in json.loads(seats):
                            ...
                            if cur_seat == 1:
                              ...
                              return response_failure(code=400, message='当前位置已经被选定，请重新选座')
                        db_seats.seat = str(db_seat)
                    else:
                        return response_failure(code=400, message='该场次没有座位')
                    # 用户钱包减掉余额
                    purse = Purse.objects.filter(user_id=user_id).first()
                    if purse:
                        if float(price) > purse.overage:
                            return response_failure(code=400, message='用户当前余额不够支付该订单')
                        else:
                            ...
                            if serializer.is_valid() is False:
                                return response_failure(code=400, message='数据库存储失败')
                    else:
                        return response_failure(code=400, message='当前用户没有充值的余额记录')
                    ...
                    if order.order_num is None:
                        return response_failure(code=400, message='获取订单编号失败')
            except Exception as e:
                raise e
        ...
        if query_params:
            try:
                ...
                if order:
                    ...
                    return response_failure(code=400, message='没有该订单信息')
            except:
                raise
